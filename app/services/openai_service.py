...
class OpenAIService:
    ...
    async def generate_panels(self, story_description: str, num_panels: int, theme: str) -> list[Panel]:
        """Generate comic panels from a story description"""
        
        # Step 1: Use GPT to break down the story into panel prompts
        panel_prompts = await self._generate_panel_prompts(story_description, num_panels, theme)
        
        # Step 2: Generate images for each prompt
        panels = await self._generate_images(panel_prompts)
        
        return panels
    
    async def _generate_panel_prompts(self, story_description: str, num_panels: int, theme: str) -> list[str]:
        ...
        gpt_response = await self.client.chat.completions.create(  
            model="gpt-4o",
            messages=[
    ...
    async def _generate_images(self, panel_prompts: list[str]) -> list[Panel]:
        ...
        for i, panel_prompt in enumerate(panel_prompts):
            ...
            try:
                ...
                if image_url:
                    ...
                    caption = await self._caption_image(image_bytes)
    ...
    async def _caption_image(self, image_bytes: bytes) -> str:
        ...
        caption = await loop.run_in_executor(None, self._generate_caption_sync, inputs)
        
        return caption
    
    def _generate_caption_sync(self, inputs: dict) -> str:
        """Synchronous caption generation (run in thread pool)"""
        with torch.no_grad():
            output_ids = self._blip_model.generate(  # type: ignore
                **inputs,
                max_length=300,
                num_beams=7,
                do_sample=True,
